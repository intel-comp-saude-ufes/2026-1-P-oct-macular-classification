...
% O babel traduz \abstractname, mas nao conhece a macro de palavras-chave da IEEEtran.
...
\begin{abstract}
A tomografia de coerência óptica é central na avaliação de condições maculares, mas a análise manual de seus cortes transversais demanda tempo especializado. Este trabalho compara, sob um mesmo protocolo, duas abordagens para a classificação automática de quatro classes: neovascularização coroideana, edema macular diabético, drusas e retina normal. A primeira é uma abordagem de aprendizado de máquina clássico, que combina descritores de histogramas de gradientes orientados com regressão logística; a segunda é uma rede convolucional compacta treinada do zero. Foi utilizada a versão 3 do conjunto de Kermany et al., preservando-se o conjunto de teste oficial e assegurando-se separação por paciente entre treino, validação e teste, além do tratamento do desbalanceamento de classes. No teste oficial, a rede convolucional alcança acurácia de 0,938, F1 macro de 0,94 e AUC macro de 0,994, ante 0,636, 0,63 e 0,864 da abordagem clássica, com desempenho superior nas quatro classes e diferença estatisticamente significativa pelo teste de McNemar. Esse resultado é obtido com cerca de 94 mil parâmetros, número significativamente menor que o da rede pré-treinada empregada por Kermany et al., que atinge acurácia 2,8 pontos percentuais superior na mesma partição. O principal modo de erro, comum às duas abordagens, é a confusão entre drusas e neovascularização coroideana, condições do mesmo espectro da degeneração macular relacionada à idade.
\end{abstract}

\begin{IEEEkeywords}
Tomografia de coerência óptica, classificação de imagens médicas, redes neurais convolucionais, aprendizado profundo, histogramas de gradientes orientados, degeneração macular relacionada à idade.
\end{IEEEkeywords}
...
A regressão logística foi implementada com o Scikit-Learn e a rede convolucional compacta em PyTorch, com semente aleatória fixa em 42 para favorecer a reprodutibilidade. A regressão logística é ajustada em etapa única sobre os descritores HOG; o restante desta seção detalha o treino da rede convolucional. Antes de entrar na rede, cada imagem é redimensionada para $224\times224$ e normalizada com média e desvio-padrão de $0{,}5$ por canal. A rede é otimizada com Adam, taxa de aprendizado inicial de $10^{-3}$, minimizando a entropia cruzada em lotes de 32 imagens por até 50 épocas.
...
O desempenho é reportado na Seção~\ref{sec:resultados} por meio de precisão, \textit{recall} e F1 para cada classe, além da média macro e da acurácia global. A média macro atribui o mesmo peso a todas as classes, independentemente de sua frequência; em particular, o \textit{recall} macro equivale à acurácia balanceada. Reporta-se ainda a área sob a curva ROC (AUC, do inglês \textit{area under the curve}) no esquema um-contra-o-resto, agregada por média macro, que resume a separabilidade das classes sem depender de um limiar de decisão. As matrizes de confusão, das quais derivam as métricas por classe, explicitam os padrões de erro entre as condições. Por fim, para verificar se a diferença de desempenho entre os dois modelos é estatisticamente significativa, aplica-se o teste de McNemar sobre as predições pareadas no conjunto de teste, que avalia a hipótese nula de igual taxa de erro considerando apenas as amostras em que os modelos discordam. O teste é adequado a este cenário porque os dois classificadores são avaliados exatamente sobre as mesmas imagens, o que torna as observações pareadas, e porque cada predição é uma variável nominal de acerto ou erro sobre uma única partição de teste. Nessas condições, o teste de McNemar é o procedimento recomendado para comparar dois classificadores~\cite{dietterich1998}, ao contrário de testes que pressupõem amostras independentes ou que exigem múltiplas medidas pareadas, como as obtidas por validação cruzada.

\section{Resultados e Discussão}
\label{sec:resultados}

A Fig.~\ref{fig:loss} apresenta as curvas de perda da rede convolucional. Ambas decrescem de forma consistente e se estabilizam a partir da décima terceira época, sem que a perda de validação volte a subir, o que caracteriza convergência sem \textit{overfitting}. A oscilação das primeiras épocas decorre da taxa de aprendizado inicial e é atenuada após as primeiras reduções aplicadas pelo agendador (Seção~\ref{subsec:treino}). O treino foi encerrado pela parada antecipada na vigésima quarta época, antes do limite configurado, e o modelo retido corresponde à décima nona época.

A ausência de \textit{overfitting} é coerente com as estratégias adotadas. A normalização em lote e o \textit{dropout}, somados ao \textit{pooling} médio global que mantém o modelo em cerca de 94 mil parâmetros (Seção~\ref{subsec:cnn}), limitam a capacidade de memorização, enquanto o aumento de dados (Seção~\ref{subsec:aumento}) introduz variação a cada época. Cabe notar que a perda de validação permanece abaixo da de treino. Esse comportamento é condizente com este protocolo: o aumento de dados e o \textit{dropout} atuam somente durante o treino, e a amostragem ponderada (Seção~\ref{subsec:desbalanceamento}) expõe a rede, no treino, a uma proporção maior das classes minoritárias, ao passo que a validação preserva a distribuição natural do conjunto.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{figures/cnn_loss_curves.png}
    \caption{Curvas de perda de treino e validação da rede convolucional compacta.}
    \label{fig:loss}
\end{figure}

A Fig.~\ref{fig:confusion} apresenta as matrizes de confusão dos dois modelos no conjunto de teste oficial. Cada matriz registra como as 250 amostras de cada classe real foram distribuídas entre as quatro classes preditas, e é dela que derivam as métricas da Tabela~\ref{tab:results}: a diagonal reúne os acertos, a leitura por linha define o \textit{recall} e a leitura por coluna, a precisão.

\begin{figure*}[t]
    \centering
    \setlength{\tabcolsep}{2pt}
    \begin{tabular}{cc}
        \includegraphics[width=0.49\textwidth]{figures/hog_logreg_confusion_matrix.png} &
        \includegraphics[width=0.49\textwidth]{figures/cnn_confusion_matrix.png} \\
        \small (a) HOG + Regressão Logística & \small (b) Rede Convolucional Compacta
    \end{tabular}
    \caption{Matrizes de confusão no conjunto de teste oficial (1.000 B-scans, 250 por classe): (a) HOG com regressão logística e (b) rede convolucional compacta. As linhas correspondem à classe real e as colunas, à classe predita.}
    \label{fig:confusion}
\end{figure*}

\begin{table}[t]
\caption{Desempenho no conjunto de teste oficial. Como o conjunto é balanceado, a acurácia coincide com a acurácia balanceada.}
\label{tab:results}
\centering
\footnotesize
\setlength{\tabcolsep}{4pt}
...
\toprule
 & \multicolumn{3}{c}{\textbf{HOG + Reg. Logística}} & \multicolumn{3}{c}{\textbf{CNN Compacta}} \\
\cmidrule(lr){2-4} \cmidrule(lr){5-7}
\textbf{Classe} & \textbf{Prec.} & \textbf{Rec.} & \textbf{F1} & \textbf{Prec.} & \textbf{Rec.} & \textbf{F1} \\
\midrule
...
\midrule
Acurácia & \multicolumn{3}{c}{0,636} & \multicolumn{3}{c}{0,938} \\
AUC macro (OvR) & \multicolumn{3}{c}{0,864} & \multicolumn{3}{c}{0,994} \\
\bottomrule
\end{tabular}
\end{table}

A rede convolucional supera a abordagem clássica por uma margem ampla e consistente: a acurácia passa de 0,636 para 0,938, o F1 macro de 0,63 para 0,94 e a AUC macro de 0,864 para 0,994. A vantagem não se concentra em uma única condição, pois o F1 da rede é superior nas quatro classes. Essa diferença é coerente com a natureza dos descritores. O HOG resume orientações locais de gradiente sobre uma grade fixa e privilegia, portanto, a geometria dos contornos; representa mal os padrões de textura e intensidade associados ao acúmulo de fluido, cuja posição varia entre exames. A rede, ao aprender os filtros diretamente dos B-scans, combina contorno, textura e contexto espacial, o que se reflete sobretudo no DME, cujo F1 salta de 0,61 para 0,97. O teste de McNemar sobre as predições pareadas no conjunto de teste rejeita a hipótese nula de igual taxa de erro com ampla margem ($p < 0{,}001$), confirmando que a superioridade da rede convolucional não decorre de flutuação amostral desta partição.

Drusas é a classe mais difícil para ambos os modelos e concentra o mesmo modo de erro. Na rede convolucional, o comportamento é assimétrico: a precisão é 1,00, ou seja, nenhuma amostra de outra classe recebe o rótulo de drusas, mas o \textit{recall} é 0,80. A Fig.~\ref{fig:confusion}(b) mostra que, das 50 drusas não recuperadas, 45 são classificadas como CNV, fluxo que explica a menor precisão da rede justamente na CNV (0,84), apesar do \textit{recall} de 0,98. O mesmo padrão, amplificado, ocorre na regressão logística com extratores HOG: das 250 drusas, apenas 108 são recuperadas e 86 migram para a CNV (Fig.~\ref{fig:confusion}(a)). A confusão é clinicamente plausível, pois drusas e CNV pertencem ao mesmo espectro da DMRI, sendo as drusas o achado da forma inicial e a CNV a manifestação neovascular avançada; casos intermediários exibem alterações sutis sob as camadas externas da retina. Contribui ainda o fato de drusas ser a classe minoritária no treino (Tabela~\ref{tab:dataset}): a amostragem ponderada equaliza a frequência das classes nos lotes, mas o faz repetindo as mesmas imagens, sem acrescentar variabilidade.

Na mesma partição de teste, a rede compacta alcança 93,8\% de acurácia, ante os 96,6\% relatados por Kermany et al.~\cite{kermany2018} com uma Inception V3 pré-treinada no ImageNet. A diferença de 2,8 pontos percentuais é obtida com cerca de 94 mil parâmetros, contra aproximadamente 23 milhões, e sem qualquer transferência de aprendizado. O valor também se aproxima dos 94,5\% da LightOCT~\cite{butola2020}, outra arquitetura compacta treinada do zero, o que reforça a viabilidade de modelos reduzidos neste domínio. Resultados obtidos na versão 2 do conjunto, como o de Arefin et al.~\cite{arefin2021}, não constituem referência equivalente, pois aquela partição apresenta sobreposição de sujeitos entre treino e teste~\cite{tampu2022}.

\section{Conclusão}
\label{sec:conclusao}

Este trabalho comparou, sob um mesmo protocolo, uma abordagem clássica baseada em descritores HOG com regressão logística e uma rede convolucional compacta treinada do zero, na classificação de quatro condições maculares em B-scans de OCT. O protocolo preservou o conjunto de teste oficial e assegurou separação por paciente entre treino, validação e teste, evitando a inflação de desempenho associada a divisões por imagem. A rede convolucional superou a abordagem clássica em todas as classes, elevando a acurácia de 0,636 para 0,938 e a AUC macro de 0,864 para 0,994, com diferença estatisticamente significativa segundo o teste de McNemar.

Esse desempenho foi obtido com cerca de 94 mil parâmetros, contra aproximadamente 23 milhões da Inception~V3 pré-treinada empregada por Kermany et al.~\cite{kermany2018}, e sem qualquer transferência de aprendizado, ficando 2,8 pontos percentuais abaixo daquele resultado na mesma partição de teste. O achado sugere que, havendo volume suficiente de dados no domínio, arquiteturas reduzidas treinadas do zero constituem alternativa viável para essa tarefa. A principal limitação é comum aos dois modelos e se concentra nas drusas, cuja maior parte dos erros é atribuída à CNV, condição do mesmo espectro da DMRI.

Reitera-se, por fim, que o escopo deste trabalho é a análise experimental de métodos de classificação. O sistema não constitui dispositivo diagnóstico nem foi validado para uso clínico, sobretudo na população brasileira, e uma eventual aplicação se restringiria ao suporte à decisão, como a triagem e a priorização de exames para revisão especializada.

% TODO: inserir Agradecimentos.
